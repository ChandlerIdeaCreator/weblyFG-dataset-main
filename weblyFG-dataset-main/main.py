...
train_data = torchvision.datasets.ImageFolder(os.path.join(data_dir, 'train_split'), transform=train_transform)
test_data = torchvision.datasets.ImageFolder(os.path.join(data_dir, 'val_split'), transform=test_transform)
...
def evaluate(test_loader, model1, model2):
    ...
    with torch.no_grad():
        ...
        for images, labels in test_loader:
            ...
            # 评估 Model 1
            logits1 = model1(images)
            # _, pred1 = torch.max(F.softmax(logits1, dim=1).data, 1) # 严格忠于原始逻辑的写法
            _, pred1 = torch.max(logits1, 1) # 这是更简洁、等效且推荐的写法
            total1 += labels.size(0)
            correct1 += (pred1 == labels_on_gpu).sum().item() # 直接在GPU上比较，更快

            # 评估 Model 2
            logits2 = model2(images)
            # _, pred2 = torch.max(F.softmax(logits2, dim=1).data, 1) # 严格忠于原始逻辑的写法
            _, pred2 = torch.max(logits2, 1) # 这是更简洁、等效且推荐的写法
            total2 += labels.size(0)
            correct2 += (pred2 == labels_on_gpu).sum().item() # 直接在GPU上比较，更快
